...
export const getEmployeeById = async (req: Request, res: Response) => {
  try {
    ...
    const emp = await svc.getById(Number(id)); 
    if (!emp) {
      return res
...
export const updateEmployee = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await svc.update(Number(id), req.body || {}); 
    if (!updated) {
      return res
        .status(HTTP_STATUS.NOT_FOUND)
        .json({ error: "Employee not found" });
    ...
    return res
      ...
      .json({ ...updated, id: Number(updated.id) });
  } catch {
    return res
      .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
...
export const deleteEmployee = async (req: Request, res: Response) => {
  try {
    ...
    const ok = await svc.remove(Number(id)); 
    if (!ok) {
      return res
        .status(HTTP_STATUS.NOT_FOUND)
        .json({ error: "Employee not found" });
...
export const getEmployeesForBranch = async (req: Request, res: Response) => {
  try {
    ...
    const list = await svc.byBranch(Number(branchId)); 
...
export const getEmployeesByDepartment = async (
  ...
  try {
    ...
    const list = await svc.listByDepartment(dept); 
    return res.status(HTTP_STATUS.OK).json(list);
  } catch {
    return res
      .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
